 ...
 * Data: GET /vendor-panel/catalog/store-types (vendor-assigned only)
 * Click opens /catalog/:catalogId products page.
 */
export default function Catalog() {
  const navigate = useNavigate()
  const [selectedId, setSelectedId] = useState(null)
  const { data: catalogStoreTypes, error, isLoading, refetch } = useVendorCatalogStoreTypes()
  const types = Array.isArray(catalogStoreTypes?.items)
    ? catalogStoreTypes.items
    : Array.isArray(catalogStoreTypes)
      ? catalogStoreTypes
      : []
  ...
  function handleSelect(type) {
    ...
    if (!id) return
    setSelectedId(id)
    navigate(`/catalog/${encodeURIComponent(id)}`)
  }
  ...
  return (
    <div className="px-[28px] pt-[26px] pb-10">
      ...
      {types.length === 0 ? (
        <p className="text-[13px] text-ink-muted">No catalogs assigned to this vendor yet.</p>
      ) : (
        <div className="grid max-w-[744px] grid-cols-1 gap-[14px] sm:grid-cols-2">
          {types.map((type) => {
            const selected =
              selectedId === type.id ||
              selectedId === type.slug ||
              catalogStoreTypes?.selectedStoreTypeId === type.id
            return (
              <button
                key={type.id}
                type="button"
              ...
              >
                ...
                <span className="min-w-0 flex-1 pt-0.5">
                  <span className="block text-[16px] font-bold leading-[1.2] text-[#1A1A1A]">
                    {type.title}
                  </span>
                  <span
                  ...
                  >
                    {type.description ||
                      (type.productCount != null
                        ? `${type.productCount} product${type.productCount === 1 ? '' : 's'}`
                        : '')}
                  </span>
                </span>
