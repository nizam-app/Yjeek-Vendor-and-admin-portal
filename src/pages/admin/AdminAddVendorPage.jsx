...
export default function AdminAddVendorPage({ onBack }) {
  ...
  const addBranch = () => {
    ...
    setBranches((prev) => [
      ...prev,
      { id: `b${Date.now()}`, name: `New branch ${n}`, detail: 'Block — · radius 5 km · ETA 30 min · min BHD 3.000' },
    ])
  }
  ...
  const addUser = () => {
    setUsers((prev) => [
      ...prev,
      { id: `u${Date.now()}`, name: 'New staff', role: 'Staff', branch: branches[0]?.name || 'All branches', status: 'Active' },
    ])
  }
  ...
  return (
    <div className="flex min-h-[calc(100vh-44px)] flex-col px-5 pb-5 pt-4 max-[700px]:px-3">
            ...
            <div className="space-y-2.5">
              {branches.map((branch) => (
                ...
                >
                  ...
                  <button type="button" className="inline-flex shrink-0 items-center gap-1 text-[12px] font-medium text-[#1aa054] hover:underline">
                    ...
                    {users.map((user) => (
                      <tr key={user.id} className="border-b border-[#f0f2f0] last:border-0">
                        ...
                        <td className="py-3 text-right">
                          <button type="button" className="grid h-8 w-8 place-items-center rounded-md text-[#8a948e] hover:bg-[#f3f5f3]" aria-label={`Actions for ${user.name}`}>
      ...
      <div className="mt-4 flex flex-wrap justify-end gap-2">
        ...
        {step === 6 ? (
          <AdminAddVendorActivateButton onClick={() => onBack()} />
        ) : (
          <button
            type="button"
            onClick={goNext}
