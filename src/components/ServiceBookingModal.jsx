...
const columnStatus = {
  new: { label: 'New', tone: 'bg-[#E3F2EB] text-[#127036]' },
  upcoming: { label: 'Upcoming', tone: 'bg-[#E7F0FF] text-[#2978DB]' },
  inProgress: { label: 'In progress', tone: 'bg-[#E3F2EB] text-[#127036]' },
}

function buildDetails(order, columnKey) {
  const status = order.noShow
    ? { label: 'No-show', tone: 'bg-[#FCE8E8] text-[#C91A24]' }
    : order.slaLabel === 'Accepted-Awaiting payment'
      ? { label: 'Awaiting payment', tone: 'bg-[#FFF4E5] text-[#D9730D]' }
      : columnStatus[columnKey] || columnStatus.new
  ...
  const durationValue = order.durationLabel || `Duration: ${order.duration || '45 mins'}`
...
export default function ServiceBookingModal({ open, onClose, order, columnKey }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative flex w-full max-w-[760px] h-[341px] flex-col overflow-hidden rounded-[16px] bg-white shadow-[0_12px_40px_rgba(0,0,0,0.18)]">
        ...
        <div className="grid grid-cols-[1.15fr_0.85fr] gap-5 px-8 py-6 max-[760px]:grid-cols-1">
          <div className="grid gap-4">
            <div>
              <p className={fieldLabel}>Duration</p>
              <p className={`${fieldValue} mt-1`}>{details.durationValue}</p>
            </div>
            <div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {details.servicesList.map((item, index) => (
                ...
                >
                  <span className="font-bold s">{item.qty}×</span>
