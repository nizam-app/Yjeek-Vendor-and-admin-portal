...
/**
 * Type-to-confirm delete for DELETE /admin/vendors/:vendorId/branches/:branchId.
 * Delete stays disabled until the typed text matches the branch name exactly.
 */
export default function AdminDeleteBranchModal({
  open,
  onClose,
  branchName = '',
  onConfirm,
  ...
  const expectedName = String(branchName || '').trim()
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <button
        type="button"
        aria-label="Close delete branch modal"
      ...
      >
        <div className="flex items-center gap-3 px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="delete-branch-title" className="text-[16px] font-bold text-[#17231c]">
              Delete branch
            </h2>
        ...
        <div className="space-y-4 px-5 py-4">
          <div className="flex items-end gap-2 rounded-[10px] bg-[#fdebec] px-3.5 py-3">
            ...
            <p className="text-[12px] leading-[16px] text-[#d64044]">
              Permanently removes{' '}
              <span className="font-semibold">{expectedName || 'this branch'}</span>
          ...
          <label className="block">
            <span className={labelClass}>
              Type <span className="font-bold text-[#d64044]">{expectedName || 'branch name'}</span>{' '}
              to confirm
            </span>
            <input
              type="text"
              ...
              placeholder={expectedName || 'Branch name'}
              autoComplete="off"
              spellCheck={false}
